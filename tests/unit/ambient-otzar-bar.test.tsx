...
import * as premiumTts from "@/lib/voice/premium-tts";
...
describe("AmbientOtzarBar — speech synthesis", () => {
 const speakSpy = () =>
  vi.spyOn(premiumTts, "speakWithOtzarVoice").mockResolvedValue({
   kind: "PREMIUM",
   provider: "XAI",
  });
 ...
 it("requests premium speak once when auto-speak is enabled (not browser TTS)", async () => {
  const spy = speakSpy();
  const user = userEvent.setup();
  renderBar();
  await user.click(screen.getByRole("region", { name: /Talk to Otzar/i }));
  await user.click(screen.getByLabelText(/Auto-speak/i));
  await user.type(screen.getByLabelText(/Message to Otzar/i), "hi");
  await user.click(screen.getByRole("button", { name: /^send$/i }));

  await waitFor(() => expect(spy).toHaveBeenCalledTimes(1));
  const [text, _fb, opts] = spy.mock.calls[0] as [
   string,
   (t: string) => void,
   { muted?: boolean },
  ];
  expect(text).toBe("Markdown response body");
  expect(text).not.toContain("**");
  expect(opts?.muted).not.toBe(true);
  expect(speakMock).not.toHaveBeenCalled();
  spy.mockRestore();
 });

 it("muting prevents premium TTS fetch AND browser speech", async () => {
  const spy = speakSpy();
  const user = userEvent.setup();
  renderBar();
  await user.click(screen.getByRole("region", { name: /Talk to Otzar/i }));
  await user.click(screen.getByLabelText(/Auto-speak/i));
  await user.click(screen.getAllByRole("button", { name: /mute otzar/i })[0]!);
  await user.type(screen.getByLabelText(/Message to Otzar/i), "hi");
  await user.click(screen.getByRole("button", { name: /^send$/i }));
  await waitFor(() => {
   expect(screen.getAllByText(/Markdown response body/).length).toBeGreaterThan(0);
  });
  // Hard mute gate: speakWithOtzarVoice never invoked while muted.
  expect(spy).not.toHaveBeenCalled();
  expect(speakMock).not.toHaveBeenCalled();
...
describe("AmbientOtzarBar — emergency TTS loop guard (Phase 12e)", () => {
 it("speaker control is present (Mute Otzar when idle)", async () => {
  const user = userEvent.setup();
  renderBar();
  await user.click(screen.getByRole("region", { name: /Talk to Otzar/i }));
  expect(
   screen.getAllByRole("button", { name: /Mute Otzar|Unmute Otzar|Stop voice/i }).length,
  ).toBeGreaterThan(0);
 });

 it("production surface has no Test Otzar voice control", async () => {
  const user = userEvent.setup();
  renderBar();
  await user.click(screen.getByRole("region", { name: /Talk to Otzar/i }));
  expect(screen.queryByRole("button", { name: /Test Otzar voice/i })).toBeNull();
 });
 ...
 it("Auto-speak toggle defaults UNCHECKED", async () => {
  ...
  renderBar();
  await user.click(screen.getByRole("region", { name: /Talk to Otzar/i }));
  const toggle = screen.getByLabelText(/Auto-speak/i) as HTMLInputElement;
...
describe("AmbientOtzarBar — voice permission + Test Otzar voice (Phase 12)", () => {
 ...
 it("does not expose provider/architecture voice copy on the primary surface", async () => {
  const user = userEvent.setup();
  renderBar();
  await user.click(screen.getByRole("region", { name: /Talk to Otzar/i }));
  const body = document.body.textContent ?? "";
  expect(body).not.toMatch(/browser STT/i);
  expect(body).not.toMatch(/premium Orion/i);
  expect(body).not.toMatch(/Test Otzar voice/i);
 });
});

describe("AmbientOtzarBar — privacy / safety copy", () => {
 it("keeps privacy under More → Voice and privacy, not primary footer", async () => {
  const user = userEvent.setup();
  renderBar();
  await user.click(screen.getByRole("region", { name: /Talk to Otzar/i }));
  // Primary surface should not dump implementation privacy slogans.
  expect(screen.queryByText(/^No raw audio is stored\.?$/i)).toBeNull();
  // Progressive disclosure still offers a plain-language privacy note.
  expect(screen.getByText(/Voice and privacy/i)).toBeTruthy();
 });
...
describe("AmbientOtzarBar — Work OS commands", () => {
 ...
 it("Follow-up artifact PERSISTS to the durable Work Ledger and shows 'Saved' (Phase 1279)", async () => {
  ...
  // Phase 1281 — View/Why shows coordination without BEAM vocabulary.
  await userEvent.setup().click(screen.getByTestId("work-artifact-open"));
  await waitFor(() =>
  ...
  ).toMatch(/tracking related owners|Coordinat/i);
  expect(
   screen.getByTestId("work-artifact-coordination").textContent,
  ).not.toMatch(/BEAM/i);
 });
 ...
 it("'Schedule a meeting with Vishesh tomorrow' (no time given) drafts a proposal asking to choose a time, no auto-create", async () => {
  ...
  expect(panel.textContent).toMatch(/drafted a meeting proposal with Vishesh|Draft meeting proposal → Vishesh/i);
 ...
 it("'Remind me to validate what I received' saves a self Work Ledger entry — never a teammate message", async () => {
  ...
  expect(panel.textContent).toMatch(/Reminder to yourself|saved that reminder for you/i);
 ...
 it("'Ask David to review the transcript' sends a governed review request via the caller's Twin, with the transcript context attached", async () => {
  ...
  expect(recordedBodies.length).toBe(0);
  expect(
   screen.getAllByText(/Sent to David/i).length,
  ).toBeGreaterThan(0);
 });
 ...
 it.skip(
  "end-to-end: context → digest → actions → save → send → tracking → correction → history → missing-context",
  async () => {
  // SKIP: multi-minute flaky mega-flow under component suite load; covered by unit slices above.
 ...
 it("shows a calm active-context indicator with a Clear control and no surveillance language", async () => {
  ...
  expect(chip.textContent).toMatch(/Current context|Using current context/i);
