...
import { api } from "../api";
import { loginErrorCopy } from "../auth/login-error-copy";
import {
...
export const useAuthStore: UseBoundStore<StoreApi<AuthState>> = create<AuthState>(
  (set) => ({
    ...
    login: async (
      ...
      if (!result.ok) {
        // Honest, recoverable copy — see login-error-copy.ts (sole-admin lockout).
        // Never show SESSION_INVALID on the login screen for credential failures.
        const message = loginErrorCopy(result.code);
