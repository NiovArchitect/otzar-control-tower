...
| C | Zoom recording ingest | **LIVE\*** | `/zoom/recordings/ingest` (`connector-data.routes.ts:106`, admin-gated, real Zoom API + WebVTT) → `ingestComms` | Full spine set | Comms etc. | unit | routed as generic TRANSCRIPT → **provenance loss + no dedupe/idempotency** (re-ingest duplicates) | R7 (partial) |
