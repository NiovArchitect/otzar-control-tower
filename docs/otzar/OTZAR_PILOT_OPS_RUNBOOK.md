...
> runs a read-only, `current_schema()`-qualified `information_schema` check BEFORE
> `app.listen`; if the six columns (or the table) are missing it **fails startup with a
> non-zero exit** and logs `INTEGRATION_CREDENTIAL_SCHEMA_INCOMPATIBLE` naming the
> missing columns + "Apply the approved additive IntegrationCredential identity schema
> before deploying this Foundation SHA." So on a code-before-schema deploy **the
> candidate process exits before listening and cannot become healthy** (it never serves
> broken reads). **Availability of the previous deployment depends on the platform's
> (Render's) deployment and rollback behavior** — do not assume the old instance stays
> live; observe and record it at deploy time. **This guard is NOT a bypass and NOT a
...
| Notification dropdown layering (MERIDIAN SIM ORG ONLY, ~1 min) | `test:e2e:live:notification-layering` (`otzar-live-notification-layering.spec.ts`; needs `OTZAR_CUSTSIM_ADMIN_PASSWORD`) | YES — Meridian sim org only: provisions one employee, seeds a real CALENDAR_EVENT_CREATED notification, logs in AS them in a browser, opens the bell, asserts the dropdown is portaled to `<body>` at `z-[70]` (above the orb `z-60`) + saves a screenshot; cleanup deletes the event + sweeps MEETING rows + suspends the identity. GREEN 2026-07-08 |
| Participant coordination / ORGX (MERIDIAN SIM ORG ONLY, ~1.5 min) | `test:e2e:live:participant-coordination` (`otzar-live-participant-coordination.spec.ts`; needs `OTZAR_CUSTSIM_ADMIN_PASSWORD`) | YES — Meridian sim org only: an OPTIONAL unresolved attendee does NOT block scheduling; a REQUIRED unresolved attendee blocks (PARTICIPANT_UNRESOLVED); the MEETING row's safe `scheduled_meeting` roster shows roles with NO ids; a non-party employee sees 0 meetings (Scheduled lane is caller-scoped). Cleanup deletes the event + sweeps MEETING rows + suspends identities. GREEN 2026-07-08 |
