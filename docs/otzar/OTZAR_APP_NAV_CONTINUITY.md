...
**UPDATE 2026-07-09 — SHIPPED.** Section 16 session continuity is LIVE (FND
`54db932` + CT `fbfd3ef`): hard-reload and protected deep-links now restore via an
HttpOnly cookie + `GET /auth/me`, and deep-link return works via
`/login?returnTo=<path>` when restore fails. So a hard reload no longer loses the
user's place. Full plan+evidence:
